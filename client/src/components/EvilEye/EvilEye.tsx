...
function generateNoiseTexture(size = 256) {
    ...
    for (let y = 0; y < size; y++) {
        for (let x = 0; x < size; x++) {
            ...
            v /= totalAmp; v = (v - 0.5) * 2.2 + 0.5; v = Math.max(0, Math.min(1, v));
...
export default function EvilEye({
    ...
    useEffect(() => {
        ...
        const section = container.closest('section'); // Find the parent section
        const target = section || container; // Fallback to container if no section found
